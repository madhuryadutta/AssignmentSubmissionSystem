    ...
    <div class="container-fluid mt-lg-5 mt-3">
        <div class="col-lg-6 container-fluid mt-3">
            <div class="card bg-white mb-3 shadow p-3 rounded h-10 mt-3">
                <form action="{{route('t_makeChange',['t_id'=>$data->t_id])}}" method="POST">
                    {{ csrf_field()}}
                    <h5 class='text-center'>Update Your Details</h5>
                    <div class="form-group row mt-3">
                        <label for="name" class="col-sm col-form-label">Name:</label>
                        <input type="text" id="name" name="user_name" class="col-sm mr-3" value="{{$data->t_name}}" required>
                    </div>
                    <div class="form-group row">
                        <label for="email" class="col-sm col-form-label">Email:</label>
                        <input type="text" id="email" name="user_email" class="col-sm mr-3" value="{{$data->t_email}}" required>
                    </div>
                    ...
                    <div class="form-group row mt-3">
                        ...
                        <select class="col-sm mr-3" name="department">
                            @foreach($data2 as $c)
                            @if($c->t_d_id == $data-t_d_id)
                            <option value="{{$c->t_d_id}}" selected>{{$c->t_d_name}}</option>
                            @else
                            <option value="{{$c->t_d_id}}">{{$c->t_d_name}}</option>
                            @endif
                            @endforeach
                        </select>
                    </div>
              ...
              <div class="col-sm-6">
                ...
                <input type="radio" id="others" name="gender" value="O" {{$data->t_gender =="O" ? "checked":""}} />
                    ...
                    <div class="form-group row">
                        ...
                        <input type="text" id="dateofbirth" name="address" class="col-sm mr-3" value="{{$data->t_address}}" required>
                    </div>
